 ...
 * @retval  
 */
void kalman1_init(struct _kalman1_state_t *state, float init_x, float init_p)
{
    state->x = init_x;
    state->p = init_p;
    state->A = 1;
    state->H = 1;
    state->q = 1e-2;//10e-6;  /* predict noise convariance */
    state->r = 0.25;//10e-5;  /* measure error convariance */
}
 ...
 *   Estimated result
 */
float kalman1_filter(struct _kalman1_state_t *state, float z_measure)
{
    /* Predict */
...
/*
 * @brief   
 *   Init fields of structure @kalman2_state.
 *   I make some defaults in this init function:
 *     A = {{1, 0.1}, {0, 1}};
 *     H = {1,0}; 
 *   and @q,@r are valued after prior tests. 
 ...
 * @retval  
 */
void kalman2_init(struct _kalman2_state_t *state, float *init_x, float (*init_p)[2])
{
    state->x[0]    = init_x[0];
    state->x[1]    = init_x[1];
 ...
 */
float kalman2_filter(struct _kalman2_state_t *state, float z_measure)
{
    float temp0 = 0.0f;
    float temp1 = 0.0f;
    float temp = 0.0f;
